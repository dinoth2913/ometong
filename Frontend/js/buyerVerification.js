/* =========================================================
   OMETONG — BUYER: IDENTITY VERIFICATION (KYC)
   Upload UI for public.buyer_verification_docs
   (supabase/export_trade_schema.sql section 14) — mirrors
   supplierVerification.js exactly (same shape, same reviewed-by-
   admin flow, same private 'verification-docs' storage bucket —
   its policies are keyed on auth.uid() alone, not role, so no new
   storage setup was needed). Reviewed on the admin side by
   adminBuyerVerification.js.
========================================================= */
document.addEventListener('DOMContentLoaded', async () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const { error } = await window.sb.from('buyer_verification_docs').insert({
      buyer_id: myId,
