/* =========================================================
   OMETONG — ADMIN: BUYER VERIFICATION DOCUMENTS (KYC review)
   Reviews what buyers submit from buyerVerification.js. Approving
   an id_document or business_license auto-flips profiles.is_verified
   via sync_buyer_verified_flag() (supabase/export_trade_schema.sql)
   - this file only ever changes buyer_verification_docs.status.

   file_url is a path in the private 'verification-docs' bucket
   (shared with the supplier flow), not a public URL — "View file"
   generates a short-lived signed URL on click rather than trying to
   render/link it directly.
========================================================= */
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const tableBody = document.getElementById('buyerVerificationDocsTableBody');
  const table = document.getElementById('buyerVerificationDocsTable');
  const emptyEl = document.getElementById('buyerVerificationDocsEmpty');
  const pendingHint = document.getElementById('buyerVerificationPendingHint');
  ...
  async function loadDocs() {
    const [docsRes, profilesRes] = await Promise.all([
      window.sb.from('buyer_verification_docs').select('*').order('created_at', { ascending: false }),
      window.sb.from('profiles').select('id, business_name, full_name')
    ]);
    if (docsRes.error) { console.error('Ometong: failed to load buyer verification docs', docsRes.error); return; }
  ...
  function buyerName(id) {
    ...
    return (p && (p.full_name || p.business_name)) || 'Unknown';
  }
    ...
    tableBody.innerHTML = docs.map(d => {
      ...
      return `
        <div class="admin-row admin-row--verification">
          <span>${esc(buyerName(d.buyer_id))}</span>
  ...
  async function decide(id, status, btn) {
    ...
    const { error } = await window.sb.from('buyer_verification_docs').update(patch).eq('id', id);
    btn.disabled = false;
    if (error) { console.error('Ometong: failed to update buyer verification doc', error); return; }
    await loadDocs();
  }
